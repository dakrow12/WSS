...
/**
 * FarSight vision for a player.
 * This vision type allows players to see a significant distance.
...
public class FarSight extends Vision {
    ...
    private static final int VISION_RANGE = 7; // FarSight players can see much further (e.g., 7 squares).
    private static final double EASTWARD_BIAS_FACTOR = 0.85; // Stronger preference for eastward paths (cost * 0.85)
    ...
    public Path easiestPath(Player player, Map map) {
        ...
        for (Direction dir : Direction.values()) {
            ...
            if (map.inBounds(nextPos.getX(), nextPos.getY())) {
                ...
                // Defensive null check for nextSquare
                if (nextSquare == null) {
                
    
                    System.err.println("Warning: map.getSquare returned null for in-bounds position: " + nextPos + " in FarSight.easiestPath");
                    continue; 
                }
